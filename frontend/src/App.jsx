...
import { logarComGoogle } from './service/firebase';
...
export default function App() {
  ...
  const handleGoogleAuth = async () => {
    ...
    setAuthMensagem('');
    try {
      const user = await logarComGoogle();
      ...
      if (!res.ok) throw new Error(data.erro || 'Erro ao comunicar com o servidor.');

      if (data.is_new) {
        setAuthEmail(data.email);
        setAuthNome(data.nome);
        setAuthMode('register');
        setAuthStep('register');
        setAuthMensagem('Conta Google vinculada! Complete os dados abaixo para finalizar.');
      } else {
      ...
      console.error(error);
      setAuthErro(error.message || 'Falha na autenticação com o Google.');
    } finally {
            ...
            <div className="p-6">
              ...
              {authStep === 'email' && (
                <form onSubmit={handleCheckEmail} className="space-y-4">
                  ...
                  <button type="button" onClick={handleGoogleAuth} disabled={isAuthLoading} className="w-full bg-white border border-gray-300 text-gray-700 font-medium py-2.5 rounded-lg hover:bg-gray-50 disabled:opacity-70 flex justify-center items-center gap-2 transition-colors shadow-sm">
